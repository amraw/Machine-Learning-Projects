...
GLOVE_DIR = "../gloVe"
PREDICTIONS_FILE = '../prediction/lstm_'
TEST_FILE = '../fnc-1-master/test_stances.csv'
OBJECT_DUMP = '../objects'
IMAGES_PATH = '../figures/lstm'

def lstm_model_2(body_length, numb_layers):
    ...
    fake_nn = models.lstm_model_2(headline_length=MAX_HEADLINE_LENGTH, body_length=MAX_BODY_LENGTH,
    ...
    lstm_list_data = []
    with open(os.path.join(OBJECT_DUMP, "lstm_"+str(body_length)+"_"+str(numb_layers)+".txt"), 'wb') as lstm_hist:
        lstm_list_data.append(fake_hist.history['acc'])
        lstm_list_data.append(fake_hist.history['val_acc'])
        lstm_list_data.append(fake_hist.history['loss'])
        lstm_list_data.append(fake_hist.history['val_loss'])
        pickle.dump(lstm_list_data, lstm_hist)
...
if __name__ == '__main__':
    ...
    lstm_model_2(body_length, numb_layers)
